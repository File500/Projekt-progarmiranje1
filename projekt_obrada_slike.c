...
typedef struct PPMImage {
	//type of picture, picture piksel data(values), 
	//temporary filed for sharpened version of picture
	char ppmType[3];
	int data[1000][1000];
	int sharpRGB[1000][1000];
	//width, height, maxValue of the picture
	unsigned int width;
	unsigned int height;
	unsigned int maxValue;
	//real height and width of array
	//every pixel is 3 numbers becouse of RGB
	int data_height;
	int data_width;
	//counters for specific color values in RGB picture
	int red_counter[256];
...
typedef struct PGMImage{
	...
	unsigned int height;
	unsigned int maxValue;
	//counter for gray values and variable to store a value of 
	//minimum occurance of specific piksel value in picture
...
void show_image_histogramsPPM(PPMImage* ppm){
	//counters for loops
	int i, j;
	//temporary counters for txt file histogram creation
	//used to determine how much characters to print for certain values of pixels
	int tmp_i=180, tmp_j=-1;
	//temporary field used to write a histogram before it is written to a txt file
	char tmp[181][256];

	FILE* color_hist = fopen("RGB_histogram.txt", "w");

	//red histogram////////////////////////////////////////////////
	//fill tmp field with '=' characters
	memset(tmp, '=', sizeof tmp);
	//set the last row of a filed to '#' characters
	//so it looks like a line
	for (int i = 0; i < 256; ++i)
	{
		tmp[tmp_i][i] = '#';
	}
	...
	//green histogram//////////////////////////////////////////////
	//reset for the wariables and tmp field so next histogram can be created
	tmp_i=180; 
	tmp_j=-1;
	memset(tmp, '=', sizeof tmp);
...
void show_cumulative_image_histogramsPPM(PPMImage* ppm){
	...
	int i, j;
	//variables for RGB counters used to store sum of all values from RGB counters
	//used for creation of cummulative histogram
...
void show_image_histogramPGM(PGMImage* pgm){
	...
	//counters used in for loops and
	//temporary counters for txt file histogram creation
	//used to determine how much characters to print for certain values of pixels
...
void show_cumulative_image_histogramPGM(PGMImage* pgm){
	//variable used to store sum of all values from gray pixel value counter
	//used for creation of cummulative histogram
	int cdfGRAY = 0;
	//counters used in for loops and
	//temporary counters for txt file histogram creation
...
int optimisePGM(PGMImage* pgm, int pos){
	...
	//value used to store decimal value of equation
	double h_val = 0.0;
	//intiger variable used to transfer h_val to int 
	int opti = 0;
	//cummulative distribution variable used to store sum  of 
	//all values fom gray counter up to the certain point determined by 'pos' variable
	int cdf = 0;
	...
	// temporary variables used to split the equyation for easier understanding
	...
	for (int i = 0; i < ppm->data_height; ++i)
	{
		for (int j = 0; j < ppm->data_width; ++j)
		{
			...
			if ((i == 0 || i == ppm->data_height - 1) || (j < 3 || j >= ppm->data_width - 3))
			{
				...
				//variables used to store current R, G, of B value and its 
				//R, G or B neghbouring values (up, down, left and right values of certain color)
				int current = ppm->data[i][j];
				int up = ppm->data[i-1][j];
				int down = ppm->data[i+1][j];
				int left = ppm->data[i][j-3];
				int right = ppm->data[i][j+3]; 
				//variable used to store new value after calculation
				int result = 5*current - left - right - up - down;
				//statement to make sure that variable ends up as positive number
				if (result < 0)
				{
					result *= -1;
				}
				//storing the new value in the field 
